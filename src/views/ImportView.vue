 ...
 * Returns `null` when the value is falsy or not a valid number.
 *
 * Callers should validate the result and surface an error / skip the row
 * when `null` is returned to avoid silently coercing bad data to `0`.
 */
...
async function processData(data: CsvRow[], fileName: string): Promise<void> {
  ...
  try {
    ...
    for (const [index, row] of data.entries()) {
      ...
      // Validate numeric fields — skip row if any are invalid
      ...
      if (quantity === null || pricePerUnit === null || totalPrice === null) {
        const invalidFields: string[] = [];
        if (quantity === null)
          invalidFields.push('ปริมาณ');
        if (pricePerUnit === null)
          invalidFields.push('ราคา');
        if (totalPrice === null)
          invalidFields.push('ราคารวม');
        errors.push(
          `ข้ามแถวที่ ${index + 2} ในไฟล์ CSV: ข้อมูลตัวเลขไม่ถูกต้อง (${invalidFields.join(', ')})`,
        );
        continue;
      }
...
<template>
  <div class="page-container">
    ...
    <div class="card import-card">
      <div class="upload-box" :class="{ 'is-dragging': isDragging }" @dragover.prevent="isDragging = true"
        ...
        <button type="button" class="btn btn-secondary upload-button" :disabled="isLoading" @click="triggerFileInput">
          เลือกไฟล์จากเครื่อง
        </button>
        <span class="file-name">{{ selectedFileName || 'ยังไม่ได้เลือกไฟล์' }}</span>
        <input ref="fileInput" type="file" accept=".csv" :disabled="isLoading" @change="handleFileUpload">
      </div>
